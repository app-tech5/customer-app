...
├── home/               Home & discovery (list, search, categories, promo banner)
├── restaurantDetail/   Restaurant page (menu, cart bar, reviews, promotions)
├── restaurantsMap/     Map / tracking (OpenStreetMap, markers)
...
| **About** | About section | Restaurant detail tabs |
| **OrderItem** | Single order line item | Cart / order flows |
...
| **CartSyncWrapper** | Syncs local cart with backend | Root navigation (signed-in tree) |
...
| **RestaurantName** | Restaurant name display | Restaurant detail |
| **RestaurantDescription** | Restaurant description | Restaurant detail |

## Conventions

- **Styling:** Components use `StyleSheet` and often import `colors` (and sometimes `currency`, `language`) from `../global`.
- **i18n:** Use `i18n` from `../lang/i18n` (or `../../lang/i18n` from nested folders).
